...
<head>
 <title>Tutorial Membuat CRUD</title>
    ...
    <div class="container">
        ...
        <a href="{{ route('mahasiswa.tambah-data') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Tambah pengumuman Baru</a>
        ...
        <table class="table">
            ...
            <tr>
                ...
                <td>{{ $d->isi }}</td>
                <td>
                    <form onsubmit="return confirm('Apakah Anda Yakin Menghapus Data ini ?');" action="{{ route('mahasiswa.destroy', $d->id) }}" method="POST">
                        <a href="{{ Route('mahasiswa.edit', $d->id) }}" class="btn btn-sm btn-primary shadow"><i class="fa fa-edit"></i> Edit</a>
                        |
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-sm btn-danger shadow"><i class="fa fa-trash"></i> Delete</button>
                        |
                        <a href="{{ Route('mahasiswa.show', $d->id) }}" class="btn btn-sm btn-secondary shadow"><i class="fa fa-info-circle"></i> Detail</a>
                    </form>
                </td>
            </tr>
            @php
